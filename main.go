...
import (
	...
	"github.com/lee20h/protoc-gen-openapi/generator"
...
func main() {
	conf := generator.Configuration{
		...
		DefaultResponse: flags.Bool("default_response", false, `add default response. If "true", automatically adds a default response to operations which use the google.rpc.Status message. Useful if you use envoy or grpc-gateway to transcode as they use this type for their default error responses.`),
	...
	opts.Run(func(plugin *protogen.Plugin) error {
		...
		if *conf.OutputMode == "source_relative" {
			for _, file := range plugin.Files {
				...
				if err := generator.NewOpenAPIv3Generator(plugin, conf, []*protogen.File{file}).Run(outputFile); err != nil {
					return err
				}
			}
		} else {
